...
void MlpackLogisticRegTableFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
    ...
    store_vector(context, resdata.model, "probabilities_0", serialize_vector(arma::conv_to<arma::vec>::from(probabilities.row(0))));
    store_vector(context, resdata.model, "probabilities_1", serialize_vector(arma::conv_to<arma::vec>::from(probabilities.row(1))));
